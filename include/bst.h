...
class BST
{
    private:
        ...
        std::function<bool(const KeyType &, const KeyType &)> m_key_less; 
        ...
        //! Returns a pointer to the BST node with the bigger key value.
        /*!
         * Helper on find_max function.
         * @param root Pointer to the root node of a tree.
         * @return A pointer to a leaf with the bigger key (right most leaf).
         */
        const BTNode * get_bigger_leaf( const BTNode * root ) const ;
